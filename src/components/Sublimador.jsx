...
import { FiUpload, FiTrash2, FiRotateCw, FiMaximize2, FiImage, FiDownload, FiZap, FiBox, FiGrid } from 'react-icons/fi'
...
function TazaCanvas2D(props) {
  ...
  }, [imagen, tamano, rotacion, posX, posY])
...
export default function Sublimador() {
  ...
  const [metodoBg, setMetodoBg] = useState('edge')
  ...
  const handleUpload = useCallback(function (e) {
    ...
    if (f && f.type.startsWith('image/')) {
      ...
      rd.onload = function (ev) { setImagen(ev.target.result); setBgRemoved(false) }
      rd.readAsDataURL(f)
    }
  }, [])
  ...
  const handleRemoveBg = useCallback(async function () {
    if (!imagen) return
    setRemovingBg(true)
    try { const res = await removeBackground(imagen, tolerancia, metodoBg); setImagen(res); setBgRemoved(true) } catch (_e) {}
    setRemovingBg(false)
  }, [imagen, tolerancia, metodoBg])

  const clearImg = function () {
    setImagen(null); setBgRemoved(false)
  ...
  return React.createElement('section', { id: 'tazas', className: 'section sublimador' },
    ...
    React.createElement('p', { className: 'section-subtitle' }, 'Subí tu imagen, quitale el fondo si es necesario y mirá cómo queda tu taza personalizada'),
      ...
      React.createElement('div', { className: 'sublimador-canvas' },
        ...
        React.createElement('div', { className: vista === '3d' ? 'canvas-area taza-area-3d' : 'canvas-area taza-area-2d' },
          vista === '3d'
            ...
            : React.createElement(TazaCanvas2D, { imagen: imagen, tamano: tamano, rotacion: rotacion, posX: posX, posY: posY, onExportRef: exportRef }),
      ...
      React.createElement('div', { className: 'sublimador-panel' },
        React.createElement('div', { className: 'panel-section' },
          ...
          imagen ? React.createElement('div', { className: 'bgmethod-selector' },
            React.createElement('button', { type: 'button', className: 'bgmethod-btn' + (metodoBg === 'edge' ? ' active' : ''), onClick: function () { setMetodoBg('edge') } },
              React.createElement(FiZap, { size: 12 }), ' Simple'),
            React.createElement('button', { type: 'button', className: 'bgmethod-btn' + (metodoBg === 'flood' ? ' active' : ''), onClick: function () { setMetodoBg('flood') } },
              React.createElement(FiZap, { size: 12 }), ' Inteligente')
          ) : null,
        ...
        React.createElement('div', { className: 'panel-section controls' },
          ...
          React.createElement('div', { className: 'control-group' },
            React.createElement('label', null, React.createElement(FiRotateCw, { size: 14 }), ' Rotación'),
            React.createElement('input', { type: 'range', min: '-180', max: '180', value: rotacion, onChange: function (e) { setRotacion(Number(e.target.value)) } }),
            React.createElement('span', null, rotacion)),
          React.createElement('div', { className: 'control-group' },
        ...
        React.createElement('div', { className: 'panel-section' },
          ...
          React.createElement('div', { className: 'taza-info' },
            ...
            React.createElement('p', null, 'La zona de impresión es de aprox. 20×8 cm sobre la cara frontal.')
          )
        )
      )
    )

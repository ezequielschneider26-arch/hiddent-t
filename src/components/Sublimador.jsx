...
import Yerbera3D from './Yerbera3D'
import './Sublimador.css'

const PRODUCTOS = {
  taza: {
    nombre: 'Taza',
    medida: 'Medida óptima de la imagen: 2000×380 px (proporción ≈ 5:1). Así llena la pared del cuerpo de la taza sin deformarse, sin llegar al fondo ni al borde.',
    detalle: [
      <React.Fragment key="a"><strong>Taza:</strong> Cerámica blanca 325ml</React.Fragment>,
      <React.Fragment key="b"><strong>Técnica:</strong> Sublimación (full color)</React.Fragment>,
      <React.Fragment key="c">El diseño envuelve toda la vuelta de la taza (360°). La manija y unos milímetros a cada lado quedan en blanco.</React.Fragment>,
    ],
    whatsapp: 'Hola! Quiero sublimar una taza personalizada.',
    archivo: 'hiddent-taza-sublimada.png',
  },
  yerbera: {
    nombre: 'Yerbera',
    medida: 'Medida óptima de la imagen: 2000×760 px (proporción ≈ 2.6:1). Así llena el cuerpo de la yerbera sin deformarse, sin llegar a la punta ni al fondo.',
    detalle: [
      <React.Fragment key="a"><strong>Yerbera:</strong> Material sintético, cierre metálico</React.Fragment>,
      <React.Fragment key="b"><strong>Técnica:</strong> Sublimación (full color)</React.Fragment>,
      <React.Fragment key="c">El diseño envuelve el cuerpo cónico (360°), dejando en blanco la punta con la anilla y el fondo.</React.Fragment>,
    ],
    whatsapp: 'Hola! Quiero sublimar una yerbera personalizada.',
    archivo: 'hiddent-yerbera-sublimada.png',
  },
}
const PRODUCTS = Object.keys(PRODUCTOS)

export default function Sublimador() {
  const [producto, setProducto] = useState('taza')
  ...
  const handleExport = function () {
    ...
    link.download = cfg.archivo
    link.href = dataUrl
    link.click()
  }

  const sendWA = function () {
    window.open('https://wa.me/5493454497729?text=' + encodeURIComponent(cfg.whatsapp), '_blank')
    handleExport()
  }

  const cambiarProducto = function (p) {
    setProducto(p)
    setImagen(null)
    if (fileInputRef.current) fileInputRef.current.value = ''
  }

  return React.createElement('section', { id: 'sublimador', className: 'section sublimador' },
    React.createElement('h2', { className: 'section-title' }, 'Sublimá tu Producto'),
    React.createElement('p', { className: 'section-subtitle' }, 'Subí tu imagen y mirá cómo queda tu producto personalizado'),

    React.createElement('div', { className: 'producto-tabs' },
      PRODUCTS.map(function (p) {
        return React.createElement('button', {
          key: p,
          className: 'producto-tab' + (p === producto ? ' active' : ''),
          onClick: function () { cambiarProducto(p) },
        }, PRODUCTOS[p].nombre)
      })
    ),
    ...
    React.createElement('div', { className: 'sublimador-main' },
      React.createElement('div', { className: 'sublimador-canvas' },
        React.createElement('div', { className: 'canvas-area taza-area-3d' },
          producto === 'taza'
            ...
            : React.createElement(Yerbera3D, { imagen: imagen, onExportRef: exportRef }),
          ...
          React.createElement('div', { className: 'canvas-tilt-hint' }, 'Arrastrá el producto para girarlo')
        ),
      ...
      React.createElement('div', { className: 'sublimador-panel' },
        React.createElement('div', { className: 'panel-section' },
          ...
          React.createElement('p', { className: 'taza-measure-hint' }, cfg.medida),
        ...
        React.createElement('div', { className: 'panel-section' },
          ...
          React.createElement('div', { className: 'taza-info' },
            cfg.detalle.map(function (d, i) { return React.createElement('p', { key: i }, d) })
          )
        )
      )
    )
